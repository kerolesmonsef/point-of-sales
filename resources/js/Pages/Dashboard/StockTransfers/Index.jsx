    ...
    return (
        <>
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                            {__("Manage stock transfers between warehouses / branches")}
                        </p>
                    </div>
                ...
                <Table.Card title={__("Stock Transfer List")}>
                    <Table>
                        <Table.Thead>
                            <tr>
                                <Table.Th>{__("Document")}</Table.Th>
                                <Table.Th>{__("Source")}</Table.Th>
                                <Table.Th>{__("Destination")}</Table.Th>
                                <Table.Th>{__("Status")}</Table.Th>
                                <Table.Th>{__("Items")}</Table.Th>
                                <Table.Th>{__("Created")}</Table.Th>
                        ...
                        <Table.Tbody>
                            {transfers.data.length > 0 ? (
                                ...
                                <Table.Empty colSpan={7} message={<div className="text-slate-500 dark:text-slate-400">{__("No stock transfers yet.")}</div>} />
                            )}
                        </Table.Tbody>
                    </Table>
                </Table.Card>
