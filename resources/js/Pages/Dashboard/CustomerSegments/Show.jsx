        ...
        <>
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400">
                            {segment.description || __("CRM customer segment")}
                        </p>
                    </div>
                    <div className="grid gap-3 sm:grid-cols-3">
                        <div className="rounded-2xl bg-white px-4 py-3 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
                            <p className="text-xs uppercase tracking-wide text-slate-500">{__("Total")}</p>
                            <p className="mt-1 text-xl font-bold text-slate-900 dark:text-white">{segment.stats.total_members}</p>
                        </div>
                        <div className="rounded-2xl bg-white px-4 py-3 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
                            <p className="text-xs uppercase tracking-wide text-slate-500">{__("Manual")}</p>
                            <p className="mt-1 text-xl font-bold text-slate-900 dark:text-white">{segment.stats.manual_members}</p>
                        </div>
                        <div className="rounded-2xl bg-white px-4 py-3 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
                            <p className="text-xs uppercase tracking-wide text-slate-500">{__("Auto")}</p>
            ...
            <div className="grid gap-6 xl:grid-cols-[1.7fr_1fr]">
                <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
                    <h2 className="mb-4 text-lg font-semibold text-slate-900 dark:text-white">{__("Segment Members")}</h2>
                    <Table>
                        <Table.Thead>
                            <tr>
                                <Table.Th>{__("Customer")}</Table.Th>
                                <Table.Th>{__("Source")}</Table.Th>
                                <Table.Th>{__("Matched")}</Table.Th>
                        ...
                        <Table.Tbody>
                            {segment.memberships.length > 0 ? (
                                ...
                                <Table.Empty colSpan={4} message={__("No segment members yet.")}>
                ...
                <div className="space-y-6">
                    {isManual && (
                        <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
                            <h2 className="mb-4 text-lg font-semibold text-slate-900 dark:text-white">{__("Add Manual Member")}</h2>
                                ...
                                >
                                    ...
                                    {customers.map((customer) => (
                                        <option key={customer.id} value={customer.id}>
                                            {customer.name} | {customer.no_telp || "-"} | {customer.is_loyalty_member ? customer.loyalty_tier : __("non-member")}
                                        </option>
                                    ))}
                                </select>
                                <button
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
                        <div className="mb-4 flex items-center gap-2">
                            ...
                            <h2 className="text-lg font-semibold text-slate-900 dark:text-white">{__("Rule Summary")}</h2>
                        </div>
                        <dl className="space-y-3 text-sm">
                            <div className="flex items-center justify-between gap-4">
                                <dt className="text-slate-500">{__("Slug")}</dt>
                                <dd className="font-medium text-slate-800 dark:text-slate-200">{segment.slug}</dd>
                            </div>
                            <div className="flex items-center justify-between gap-4">
                                <dt className="text-slate-500">{__("Rule Type")}</dt>
                                <dd className="font-medium text-slate-800 dark:text-slate-200">{segment.auto_rule_type || "-"}</dd>
                            </div>
                            <div className="flex items-center justify-between gap-4">
                                <dt className="text-slate-500">{__("Status")}</dt>
                                <dd className="font-medium text-slate-800 dark:text-slate-200">{segment.is_active ? __("Active") : __("Inactive")}</dd>
                            </div>
                        </dl>
                    </div>
                </div>
