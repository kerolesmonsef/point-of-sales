...
export default function PriceLists({ priceLists }) {
    ...
    const handleDelete = (pl) => {
        if (!confirm(__("Delete price list") + " " + pl.name + "?")) return;
        router.delete(route("price-lists.destroy", pl.id));
    };

    const scopeLabel = { all: __("All"), walk_in: __("Walk-in"), registered: __("Registered"), member: __("Member"), segment: __("Segment") };

    return (
        <>
            <Head title={__("Price List")} />
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400">{__("Special prices per customer group")}</p>
                    </div>
                    <button onClick={() => { resetForm(); setShowForm(true); }} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary-500 hover:bg-primary-600 text-white text-sm font-medium transition-colors">
                        <IconPlus size={18} /> {__("New")}
                    </button>
                </div>

                {showForm && (
                    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5">
                        <h3 className="font-semibold mb-4">{editing ? __("Edit Price List") : __("New Price List")}</h3>
                        <form onSubmit={handleSubmit} className="space-y-4 max-w-lg">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium mb-1">{__("Group")}</label>
                            ...
                            <div>
                                <label className="block text-sm font-medium mb-1">{__("Notes")}</label>
                            ...
                            <div className="flex gap-3">
                                <button type="submit" className="px-4 py-2 rounded-xl bg-primary-500 text-white text-sm font-medium">{editing ? __("Update") : __("Save")}</button>
                                <button type="button" onClick={resetForm} className="px-4 py-2 rounded-xl border text-sm font-medium">{__("Cancel")}</button>
                            </div>
                        </form>
                    </div>
                )}
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                    {priceLists.length > 0 ? (
                            ...
                            {priceLists.map(pl => (
                                <div key={pl.id} className="p-4 flex items-center gap-4">
                                    <div className="flex-1">
                                        ...
                                        <p className="text-sm text-slate-500">{scopeLabel[pl.customer_scope] || pl.customer_scope} • {pl.items_count} {__("products")} • {__("Priority")} {pl.priority}</p>
                                    </div>
                        ...
                        <div className="p-8 text-center text-slate-400">{__("No price lists yet.")}</div>
                    )}
                </div>
            </div>
        </>
