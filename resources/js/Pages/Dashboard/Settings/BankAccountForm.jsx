...
export default function BankAccountForm({ bankAccount = null }) {
    ...
    return (
        <>
            <Head title={isEdit ? __("Edit Bank Account") : __("Add Bank Account")} />
            <div className="max-w-3xl space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                            <IconBuildingBank size={28} className="text-primary-500" />
                            {isEdit ? __("Edit Bank Account") : __("Add Bank Account")}
                        </h1>
                        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                            {__("Enter bank account details for transfer payments.")}
                        </p>
                    </div>
                    <Link
                ...
                >
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Input
                            label={__("Bank Name")}
                            placeholder={__("e.g. BCA, Mandiri, BNI")}
