...
export default function DiscountApprovals({ pendingTransactions }) {
    const confirm = (action, tx) => {
        if (!window.confirm(`${action === "approve" ? __("Approve") : __("Deny")} ${__("discount transaction")} ${tx.invoice}?`)) return;
        router.post(route(`discount-approvals.${action}`, tx.id), {}, {
            onSuccess: () => toast.success(action === "approve" ? __("Discount approved") : __("Discount denied")),
            onError: () => toast.error(__("Failed")),
        });
    };

    return (
        <>
            <Head title={__("Discount Approval")} />
            <div className="space-y-6">
                <div>
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                        {pendingTransactions.length} {__("transactions waiting for discount approval")}
                    </p>
                </div>
                ...
                {pendingTransactions.length > 0 ? (
                            ...
                            <div key={tx.id} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5">
                                <div className="flex items-start justify-between">
                                    <div>
                                        ...
                                        <p className="text-sm text-slate-500 dark:text-slate-400">{__("Cashier")}: {tx.cashier}</p>
                                        <p className="text-sm text-slate-500 dark:text-slate-400">{__("Customer")}: {tx.customer}</p>
                                    </div>
                                ...
                                <div className="mt-3 flex items-center gap-4 text-sm">
                                    <span className="text-slate-500">{__("Discount")}: <strong className="text-danger-500">{formatCurrency(tx.discount)}</strong></span>
                                    <span className="text-slate-500">{__("Total")}: <strong>{formatCurrency(tx.grand_total)}</strong></span>
                                </div>
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-8 text-center">
                        ...
                        <p className="text-slate-500 dark:text-slate-400">{__("All transactions have been verified.")}</p>
                    </div>
                )}
            </div>
        </>
