...
Read by opencode directly. Claude Code reads it via `CLAUDE.md`, which imports this file with `@AGENTS.md` — keep instructions here, not duplicated in both.
...
**Keep in sync:** when a feature is added or changed, update its file under `docs/features/*.md` (and add a row to `docs/feature-index.md` if it's a new module) in the same change. Docs describe current shipped behavior, not the plan — if a feature is only partially done (e.g. backend ready but no UI yet), say so instead of marking it done.
